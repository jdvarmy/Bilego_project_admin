...
const Events = inject(
  ...
  observer((props) => {
    ...
    const {
      eventsStore: { filters, events, isLoading, clearFilters },
    } = props;
    ...
    useEffect(() => {
      const {
        eventsStore: { getEventsOfUser, initState },
        securityStore: { user },
      } = props;
      initState();

      if (user?.email) {
        getEventsOfUser(user.email);
      }
    ...
    const columns = [
      {
        title: (
          <div className={classes.flex}>
            <div>Название</div>
            {filters.events !== undefined && (
              <CloseCircleOutlined
                onClick={() => handleSearchBase({ events: undefined })}
              />
            )}
          </div>
        ),
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  value && value.length > 1
                    ...
                    : filters.events !== undefined &&
                      handleSearchBase({ events: undefined });
                }}
        ...
        filterIcon: () => (
          <SearchOutlined
            style={{ color: filters.events ? "rgb(246, 37, 90)" : undefined }}
          />
        ),
        ...
        title: (
          <div className={classes.flex}>
            ...
            {(filters.startDate !== undefined ||
              ...
              <CloseCircleOutlined
                onClick={() =>
                  handleSearchBase({ startDate: undefined, endDate: undefined })
                }
              />
            )}
          </div>
            ...
            <div className={classes.filter}>
              <RangePicker
                ...
                onCalendarChange={(dates) => {
                  if (
                    dates[0] === filters.startDate &&
                    dates[1] === filters.endDate
                  )
                    return;
                  handleSearchBase(
          ...
          const menu = (
            <Menu>
              <Menu.Item
                key="1"
                onClick={() =>
                  ExportCSV(csvData, `Отчет по билетам ${data.event} bilego`)
                }
              >
                ...
                onClick={() =>
                  setModal({
                    ...
                    content: (
                      <>
                        <Typography>
                          Скопируйте ссылку ниже и вставьте в любое место Вашего
                          сайта:
                        </Typography>
                        <Input.TextArea
                          rows={3}
                          className={classes.textarea}
                          value={`<iframe src="${data.ticketLink}" width="100%" height="645px"></iframe>`}
                        />
                      </>
                    ),
                  })
              ...
              >
                Сформировать ссылку на событие
              </Menu.Item>
            </Menu>
          );
          return (
    ...
    const xlsxFileName = () => {
      ...
      return startDate && endDate
        ? `События с ${startDate.format(format)} по ${endDate.format(
            format
          )}-bilego`
        : !startDate && endDate
    ...
    return (
      <div>
        <PageHeader
          ...
          extra={[
            <Button key="2" onClick={clearFilters}>
              Очистить фильтры
            </Button>,
            <Button
              key="1"
              type="primary"
              onClick={() => ExportCSV(xlsxFileData(), xlsxFileName())}
            >
              Скачать отчет
            </Button>,
          ]}
        ...
        <Table
          ...
          expandable={{
            expandedRowRender: (orders) => (
              <OrderDetails orders={orders.ordersInfo} />
            ),
          }}
          bordered={true}
          pagination={false}
        ...
        <Modal
          ...
          footer={null}
          onCancel={() =>
            setModal({ title: null, link: null, visible: null, content: null })
          }
        >
          {modal.content}
        </Modal>
